...
import "../base/MultiSigWalletConstants.sol";
...
contract MultiSigWallet is IMultiSigWallet {
    ...
    constructor(
        ...
        for (uint i = 0; i < _owners.length; i++) {
            require(
                ...
                "Invalid owner."
            ); // check if owner is valid or already exists
    ...
    function removeOwner(address owner) public onlyWallet ownerExists(owner) {
        ...
        if (required > owners.length) changeRequirement(owners.length); // change requirement if required is greater than owners
    ...
    function submitTransaction(
        ...
        uint256 amount,
        bytes memory data
    ) external override returns (uint transactionId) {
